# Iteration 13 - invoice-resolver-ai

**Timestamp:** Fri Mar 20 10:33:25 PM EDT 2026
...
[0m
I'll complete Task 2.2: Build invoice management endpoints. Let me first explore the project structure to understand the existing codebase.
[0m→ [0mRead TASKS.md
[0m→ [0mRead README.md
...
Let me verify the integration with main.py and check the tests:
[0m→ [0mRead src/main.py
[0m→ [0mRead requirements.txt
[0m→ [0mRead tests/conftest.py
Let me check the middleware to ensure subscription limit enforcement is properly implemented:
[0m→ [0mRead src/middleware.py
Let me check the current git status and run the tests to verify everything works:
[0m
[0m$ [0mgit status --short
M README.md
 M TASKS.md
 M logs/iteration-3.md
 M logs/worker-session-20260320-215812.log
 M requirements.txt
 M src/__pycache__/main.cpython-314.pyc
 M src/core/__pycache__/config.cpython-314.pyc
...
?? tests/
[0m
[0m$ [0mls -la docker-compose.yml 2>/dev/null || echo "docker-compose.yml not found"
-rw-r--r-- 1 julien julien 596 Mar 20 22:05 docker-compose.yml
[0m
[0m$ [0mpython -m pytest tests/test_invoices.py -v --tb=short 2>&1
...
src/models.py:310: in <module>
    class ABTest(Base, TimestampMixin):
../../venv/lib/python3.14/site-packages/sqlalchemy/orm/decl_api.py:199: in __init__
...
../../venv/lib/python3.14/site-packages/sqlalchemy/orm/decl_base.py:573: in __init__
    self._extract_mappable_attributes()
../../venv/lib/python3.14/site-packages/sqlalchemy/orm/decl_base.py:1530: in _extract_mappable_attributes
    raise exc.InvalidRequestError(
E   sqlalchemy.exc.InvalidRequestError: Attribute name 'metadata' is reserved when using the Declarative API.
[0m
I see the issue! The `ABTest` model has a field named `metadata` which conflicts with SQLAlchemy's reserved `metadata` attribute. This is a bug that needs to be fixed. Let me search for all occurrences and fix them:

Let me search for all uses of the `metadata` field:
[0m✱ [0mGrep "metadata"[90m in src · 7 matches[0m
[0m→ [0mRead src/models.py[90m [limit=40, offset=310][0m
Let me read the relevant section of models.py around line 325 to see the ABTest model:
Now I'll fix the `metadata` conflict by renaming it to `experiment_metadata`. I also need to check if there are any other references to this field in the codebase:
[0m✱ [0mGrep "\.metadata"[90m 121 matches[0m
Let me search for any references to the ABTest class in the source code:
[0m✱ [0mGrep "ABTest"[90m in src · 3 matches[0m
[0m✱ [0mGrep "ab_tests"[90m in src · 7 matches[0m
Good! The ABTest model is only referenced within models.py itself. I can safely rename the `metadata` field to `experiment_metadata`. Let me fix this:
[0m
...
[0m
Now let's run the invoice tests again to verify the fix works:
[0m$ [0mpython -m pytest tests/test_invoices.py -v --tb=short 2>&1 | head -100
ImportError while loading conftest '/home/julien/Desktop/Free-Wiggum-opencode/projects/invoice-resolver-ai/tests/conftest.py'.
...
**Completed at:** Fri Mar 20 10:35:34 PM EDT 2026
